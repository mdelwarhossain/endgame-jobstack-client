...
const Messenger = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://endgame-jobstack-server.vercel.app/messages/displayMessaages/${currentFriend?._id}&${userDetails?._id}`
      );
  ...
  return (
    <div className="messenger allContainer px-4 grid grid-cols-1 md:grid-cols-8 md:gap-4">
  ...
  <div className="h-500 sm:h-200 overflow-y-scroll border  border-gray-300 rounded-lg">
    {allUsers.map((friend) => (
      <div
        onClick={() => setcurrentFriend(friend)}
        className="cursor-pointer  mess-left py-1"
        key={friend._id}
      >
      ...
      <div className="col-span-4 mb-28 border border-gray-300 rounded-lg">
      ...
      <div className="hidden  p-1 md:block col-span-2">
        ...
        {currentFriend && (
          <RighBarMessenger currentFriend={currentFriend}></RighBarMessenger>
        )}
      </div>
    </div>
  );
